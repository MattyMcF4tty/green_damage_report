...
interface WitnessListProps {
    ...
    onChange: (witnesses: {name:string, phone:string, email:string}[]) => void;
}
...
const WitnessList = ({value, onChange}: WitnessListProps) => {
    ...
    const [witnesses, setWitnesses] = useState<{name:string, phone:string, email:string}[]>(value);
            ...
            <div className="">
                ...
                <div>
                    {witnesses.map((witness, index) => (
                        <button key={index} type="button" onClick={() => handleShowUpdateWitness(index)}
...
interface newWitnessPopUpProps {
    ...
    setWitnesses: (witnesses: {name: string, phone: string, email: string}[]) => void;
    showPopUp: (show: boolean) => void;
}

const NewWitnessPopUp = ({showPopUp, setWitnesses, witnesses}: newWitnessPopUpProps) => {
    ...
    const handleNewWitness = () => {
        const newWitness = {name: witnessName, phone: witnessPhone, email: witnessEmail}
    ...
    return (
        <div
        className="fixed flex justify-center items-center z-20 inset-0 bg-black bg-opacity-75 overflow-y-auto scro"
        >
            <div className="absolute flex flex-col justify-center bg-white p-4 rounded-lg">
                <div className="flex flex-row border-b-[1px] border-gray-300 mb-4 justify-center">
                    <button onClick={() => handleClosePopUp()} type="button" className="absolute ml-4 pr-4 left-0">X</button>
                ...
                className="bg-MainGreen-300 text-white mt-4 p-2">Save</button>
            </div>
        </div>
    )
}
...
const UpdateWitnessPopUp = ({showPopUp, setWitnesses, witnesses, index}: UpdateWitnessPopUpProps) => {

    const [witnessName, setWitnessName] = useState<string>(witnesses[index].name)
    const [witnessPhone, setWitnessPhone] = useState<string>(witnesses[index].phone)
    const [witnessEmail, setWitnessEmail] = useState<string>(witnesses[index].email)


    const handleUpdateWitness = () => {
        const updatedWitness = {name: witnessName, phone: witnessPhone, email: witnessEmail}
    ...
    return (
        ...
        >
            <div className="absolute flex flex-col justify-center bg-white p-4 rounded-lg">
                <div className="flex flex-row border-b-[1px] border-gray-300 mb-4 justify-center">
                    <button onClick={() => showPopUp(false)} type="button" className="absolute ml-4 pr-4 left-0">X</button>
                    <h1 className="">New witness</h1> 
                </div>
                ...
                <div className="flex flex-row justify-evenly">
                    <button type="button" onClick={() => handleDeleteWitness()}
                    className="bg-red-600 text-white mt-4 p-2 w-1/3">Delete</button>
                    <button type="button" onClick={() => handleUpdateWitness()}
                    className="bg-MainGreen-300 text-white mt-4 p-2 w-1/3">Update</button>
                </div>
            </div>
        </div>
    )
