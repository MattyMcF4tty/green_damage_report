...
export const encryptReport = (reportData: CustomerDamageReport) => {
  ...
  try {
    ...
    if (driverInfo.socialSecurityNumber) {
      driverInfo.socialSecurityNumber = lowEncryptText(
        driverInfo.socialSecurityNumber
      );
    }
    if (driverInfo.drivingLicenseNumber) {
      driverInfo.drivingLicenseNumber = lowEncryptText(
      ...
      bikerInfo.map((biker, index) => {
        const newBiker = {
          ...
          personDamage: biker.personDamage
            ? lowEncryptText(biker.personDamage)
            : biker.personDamage,
        };
      ...
      vehicleInfo.map((vehicle, index) => {
        const encryptedVehicle = {
          ...
          driversLicenseNumber: vehicle.driversLicenseNumber
            ? lowEncryptText(vehicle.driversLicenseNumber)
            : vehicle.driversLicenseNumber,
          insurance: vehicle.insurance
            ? lowEncryptText(vehicle.insurance)
            : vehicle.insurance,
          numberplate: vehicle.numberplate
            ? lowEncryptText(vehicle.numberplate)
            : vehicle.numberplate,
      ...
      pedestrianInfo.map((pedestrian, index) => {
        const encryptedPedestrian = {
          name: pedestrian.name
            ? lowEncryptText(pedestrian.name)
            : pedestrian.name,
          phone: pedestrian.phone
            ? lowEncryptText(pedestrian.phone)
            : pedestrian.phone,
          email: pedestrian.email
            ? lowEncryptText(pedestrian.email)
            : pedestrian.email,
          personDamage: pedestrian.personDamage
            ? lowEncryptText(pedestrian.personDamage)
      ...
      otherObjectInfo.map((otherInfo, index) => {
        const encryptedOtherInfo = {
          description: otherInfo.description
            ? lowEncryptText(otherInfo.description)
            : otherInfo.description,
          information: otherInfo.information
            ? lowEncryptText(otherInfo.information)
...
export const decryptReport = (
  ...
  try {
    ...
    if (driverInfo.socialSecurityNumber) {
      driverInfo.socialSecurityNumber = lowDecryptText(
        driverInfo.socialSecurityNumber
      );
    }
    if (driverInfo.drivingLicenseNumber) {
      driverInfo.drivingLicenseNumber = lowDecryptText(
        driverInfo.drivingLicenseNumber
      );
    }
    if (driverInfo.phoneNumber) {
    ...
    if (bikerInfo.length > 0) {
      bikerInfo.map((biker, index) => {
        const newBiker = {
          ...
          ebike: biker.ebike,
          personDamage: biker.personDamage
            ? lowDecryptText(biker.personDamage)
            : biker.personDamage,
        };
        bikerInfo[index] = newBiker;
      });
    ...
    if (vehicleInfo.length > 0) {
      vehicleInfo.map((vehicle, index) => {
        const decryptedVehicle = {
          ...
          driversLicenseNumber: vehicle.driversLicenseNumber
            ? lowDecryptText(vehicle.driversLicenseNumber)
            : vehicle.driversLicenseNumber,
          insurance: vehicle.insurance
            ? lowDecryptText(vehicle.insurance)
            : vehicle.insurance,
          numberplate: vehicle.numberplate
            ? lowDecryptText(vehicle.numberplate)
            : vehicle.numberplate,
    ...
    if (pedestrianInfo.length > 0) {
      pedestrianInfo.map((pedestrian, index) => {
        const decryptedPedestrian = {
          name: pedestrian.name
            ? lowDecryptText(pedestrian.name)
            : pedestrian.name,
          phone: pedestrian.phone
            ? lowDecryptText(pedestrian.phone)
            : pedestrian.phone,
          email: pedestrian.email
            ? lowDecryptText(pedestrian.email)
            : pedestrian.email,
          personDamage: pedestrian.personDamage
            ? lowDecryptText(pedestrian.personDamage)
            : pedestrian.personDamage,
        };
    ...
    if (otherObjectInfo.length > 0) {
      otherObjectInfo.map((otherInfo, index) => {
        const decryptedOtherInfo = {
          description: otherInfo.description
            ? lowDecryptText(otherInfo.description)
            : otherInfo.description,
          information: otherInfo.information
            ? lowDecryptText(otherInfo.information)
            : otherInfo.information,
        };
  ...
  try {
    decryptedReport.updateFields({
      userPhoneNumber: decryptedReport.userPhoneNumber
        ? lowDecryptText(decryptedReport.userPhoneNumber)
        : decryptedReport.userPhoneNumber,
      policeReportNumber: decryptedReport.policeReportNumber
        ? lowDecryptText(decryptedReport.policeReportNumber)
        : decryptedReport.userPhoneNumber,
    });
  } catch (error: any) {
    throw error;
