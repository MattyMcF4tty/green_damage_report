...
export const getFirebaseAdmin = () => {
    ...
    if (!firebaseAdmin) {
        try {
            ...
            if (!privateKey) {
                throw new AppError('INTERNAL_ERROR', 'FIREBASE_ADMIN_PRIVATE_KEY is not defined in enviroment.')
            }            
            ...
            console.error("Something went wrong initializing the Firebase Admin SDK:\n", error);
...
export const getAdminStorage = () => {
    try {
        ...
        throw new AppError('INTERNAL_ERROR', `Error getting Firebase admin storage bucket: ${error.message}`)
    }
}
