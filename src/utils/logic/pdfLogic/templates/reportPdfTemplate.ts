...
import {
	fetchAdminDamageReport,
	requestDamageReportFileDownload,
	requestDamageReportFolderDownload,
} from '../../damageReportLogic.ts/apiRoutes';
...
const createReportPDF = async (reportId: string) => {
	...
	const phoneNumber = data.driverInfo.phoneNumber ? data.driverInfo.phoneNumber : '-';

	const email = data.driverInfo.email ? data.driverInfo.email : 'No email has been provided';
	...
	const renterBirthday = data.renterInfo.birthDate ? `${data.renterInfo.birthDate}` : '-';
	...
	const addDriverInfoToPDF = (doc: jsPDF, startY: number, data: AdminDamageReport) => {
		...
		const addressLines = doc.splitTextToSize(`Address: ${address}`, maxLocationWidth);
		...
		doc.text(`Social security number: ${socialSecurityNumber}`, 105, driverInfoY + 24);
		doc.text(`Driving License Number: ${drivingLicenseNumber}`, 105, driverInfoY + 32);
	...
	doc.text(`${data.accidentAddress || '-'}`, accidentInfoRightX, accidentInfoY + 26);
	...
	if (accidentDescriptionLines.length > 0) {
		...
		doc.text('No accident description has been provided', accidentInfoRightX, accidentInfoY + 49);
	}
	// Damage information
	...
	const bikerInfoHeight = data.bikerInfo.length > 0 ? data.bikerInfo.length * 3 + 20 : 0;
	...
	const spaceForBikerInfo = currentY + bikerInfoHeight < doc.internal.pageSize.height;
	...
	if (data.bikerInfo && data.bikerInfo.length > 0) {
		...
		data.bikerInfo.forEach((currentBiker, index) => {
			...
			const injuryDescriptionHeight = numInjuryDescriptionLines * damageLineHeight;
			...
			// Biker information header
			doc.setFont('bold');
			doc.text('Biker Information on other party involved in incident', 15, currentY + 10);
			doc.setLineWidth(0.5);
			...
			doc.text(
				`Electric bike: ${
					currentBiker.ebike !== null ? (currentBiker.ebike === true ? 'Yes' : 'No') : '-'
				}`,
				100,
				currentY + 28,
			);
			...
			for (let i = 0; i < numInjuryDescriptionLines; i++) {
				doc.text(injuryDescriptionLines[i], 100, currentY + 44 + i * damageLineHeight);
			}

			// Update currentY for the next biker
			currentY += bikerInfoHeight + sectionSpacing + injuryDescriptionHeight + 2;
		});
	...
	if (data.vehicleInfo && data.vehicleInfo.length > 0) {
		...
		data.vehicleInfo.forEach((currentVehicle, index) => {
			...
			doc.setFillColor('#E6EEE5');
			doc.roundedRect(10, currentY, 190, vehicleInfoHeight + sectionSpacing + 10, 5, 5, 'F');

			// Vehicle information header
			doc.setFont('bold');
			doc.text('Vehicle Information on other party involved in incident', 15, currentY + 10);
			doc.setLineWidth(0.5);
			...
			doc.text(`License Number: ${currentVehicle.driversLicenseNumber || '-'}`, 100, currentY + 28);
	...
	if (data.pedestrianInfo && data.pedestrianInfo.length > 0) {
		...
		const currentPedestrianSectionHeight = pedestrianInfoHeight + sectionSpacing;
		...
		data.pedestrianInfo.forEach((currentPedestrian, index) => {
			...
			doc.setFont('bold');
			doc.text('Pedestrian Information on other party involved in incident', 15, currentY + 10);
			doc.setLineWidth(0.5);
	...
	if (data.otherObjectInfo && data.otherObjectInfo.length > 0) {
		...
		data.otherObjectInfo.forEach((currentOther, index) => {
			...
			const numDescriptionLines = Math.min(descriptionMaxLines, descriptionLines.length);
			...
			const numInformationLines = Math.min(informationMaxLines, informationLines.length);
			const informationHeight = numInformationLines * damageLineHeight;

			// Calculate the total other section height with additional spacing
			const currentOtherSectionHeight = otherInfoHeight + sectionSpacing + descriptionHeight;
			...
			doc.setFillColor('#E6EEE5');
			doc.roundedRect(10, currentY, 190, currentOtherSectionHeight + 5, 5, 5, 'F');

			// Other information header
			doc.setFont('bold');
			doc.text('Other Information on other party involved in incident', 15, currentY + 10);
			doc.setLineWidth(0.5);
			...
			for (let i = 0; i < numInformationLines; i++) {
				doc.text(informationLines[i], 100, currentY + 36 + i * damageLineHeight);
			}
	...
	const getCorrectlyOrientedImage = async (imgBase64: string): Promise<string> => {
	...
	const addImageWithSpacingCheck = async (
		...
		doc.addImage(imageBase64, format, x, currentY, width, height, undefined, undefined);
	...
	function calculateDamageSpaceRequired(damageDescriptionHeight: number): number {
	...
	if (data.damages) {
		for (const damage of data.damages) {
			...
			if (
				currentY + calculateDamageSpaceRequired(damagePositionDescriptionHeight) >
			...
			doc.setFont('bold');
			doc.text(`Damage to ${position?.toLowerCase()} of GreenMobility car`, 15, currentY + 10); // Use currentY
			doc.setLineWidth(0.5);
